...
let rec make_list curr_list = function 
  ...
  | h::_ when h = LEFT_SQUARE -> (raise (BadListError "Shouldn't have another bracket"))
...
let rec populate_json curr_json_rec = function  
  ...
  | _::t -> populate_json curr_json_rec t
;;
