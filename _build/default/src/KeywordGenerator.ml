...
let rec add_to_pq (key, value) pq = 
  match pq with  
  | [] -> (key, value)::[]
  | (_, hval)::_ when value > hval -> (key, value)::pq
  | (hkey, hval)::_ when value == hval && ((String.length key) > (String.length hkey)) -> (key, value)::pq
...
let rec top_n_keywords top_n n = function
  | (hkey, _)::t when n > 0 -> (hkey::top_n_keywords top_n (n - 1) t)
  | _ -> top_n
